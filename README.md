<div align="center">

<img src="https://github.com/SP-XD/SP-XD/blob/main/images/hellocoders_rounded.gif?raw=true" width="60%" />

# 👋 Hi, I'm Mohammed Sami Al-Jardan

### Flutter Developer | Firebase Enthusiast | Mobile App Developer

<img src="https://github.com/SP-XD/SP-XD/blob/main/images/dev-working_rounded.gif?raw=true" width="35%" />

<p>
I am passionate about building modern, high-quality mobile applications using Flutter and Firebase.
I enjoy learning new technologies, solving real-world problems, and continuously improving my development skills.
</p>

![](https://komarev.com/ghpvc/?username=YOUR_GITHUB_USERNAME&style=flat&color=blue&label=PROFILE+VIEWS)

</div>

---

## 🚀 Tech Stack

![Flutter](https://img.shields.io/badge/Flutter-02569B?style=flat&logo=flutter&logoColor=white)
![Dart](https://img.shields.io/badge/Dart-0175C2?style=flat&logo=dart&logoColor=white)
![Firebase](https://img.shields.io/badge/Firebase-FFCA28?style=flat&logo=firebase&logoColor=black)
![SQLite](https://img.shields.io/badge/SQLite-07405E?style=flat&logo=sqlite&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![C#](https://img.shields.io/badge/C%23-239120?style=flat&logo=csharp&logoColor=white)
![.NET](https://img.shields.io/badge/.NET-512BD4?style=flat&logo=dotnet&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)
![VS Code](https://img.shields.io/badge/VS_Code-007ACC?style=flat&logo=visual-studio-code&logoColor=white)

---

## 💻 About Me

```dart
class MohammedSami extends Developer {

  final String role = "Flutter Developer";

  final List<String> skills = [
    "Flutter",
    "Firebase",
    "SQLite",
    "HTML",
    "CSS",
    "JavaScript",
    ".NET",
    "Git",
    "GitHub"
  ];

  final List<String> currentlyLearning = [
    "Clean Architecture",
    "Advanced Flutter",
    ".NET",
    "Backend Development"
  ];

  final String goal =
      "Build modern, scalable and user-friendly mobile applications.";
}
```

### 🌱 Currently Learning

- Flutter Advanced
- Firebase
- .NET
- Clean Architecture

### 🎯 Interests

- 📱 Mobile App Development
- ☁️ Firebase Backend
- 🎨 UI/UX Design
- 🚀 Software Engineering

---

<div align="center">

### ⭐ Thanks for visiting my profile!

*"Code. Learn. Build. Improve. Repeat."* 🚀

</div>
